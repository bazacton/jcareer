...
class JobHunt_Job_Alerts {
    ...
    public function __construct() {
        // Define constants
        define('JOBHUNT_NOTIFICATIONS_PLUGIN_VERSION', '3.2');
            ...
            if ($selected_frequency != false) {
                ...
                if (time() > strtotime($selected_frequency, intval($last_time_email_sent))) {
                    ...
                    if (isset($template['email_notification']) && $template['email_notification'] == 1 && self::get_job_alerts_count(self::$job_details['jobs_query'], self::$job_details['frequency']) > 0) {
                        ...
                        $from = (isset($template['from']) && $template['from'] != '') ? $template['from'] : esc_attr(get_bloginfo('name')) . ' <' . get_bloginfo('admin_email') . '>';
...
//
    ...
    public static function get_job_alerts_count($jobs_query, $frequency) {
        ...
        $jobs_query1['meta_query'][] = array(
            'key' => 'cs_job_posted',
            'value' => strtotime(date('Y-m-d H:i:s', strtotime($frequency))),
            'compare' => '>=',
        );
        $jobs_query1['meta_query'][] = array(
            'key' => 'cs_job_status',
        ...
        $jobs_query1['posts_per_page'] = -1;
        
        $loop_count = new WP_Query($jobs_query1);
