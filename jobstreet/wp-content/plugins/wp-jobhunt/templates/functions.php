...
if (!function_exists('cs_hide_comment_form_callback')) {

    function cs_hide_comment_form_callback($name) {

        remove_filter('comment_form_field_comment', 'jobcareer_form_field_comment');
    }

    add_action('cs_hide_comment_form', 'cs_hide_comment_form_callback');
}

//add metaboxes
if (!function_exists('cs_metaboxes_adding')) {

    function cs_metaboxes_adding($name) {
...
if (!function_exists('jobcareer_contact_form_submit')) {
    ...
    function jobcareer_contact_form_submit() {
        ...
        $cs_contact_email = isset( $_POST['cs_contact_email'] )? $_POST['cs_contact_email'] : '';

        if (wp_mail($cs_contact_email, $contact_subject, $message, $headers)) {
            $json = array();
            $json['type'] = "success";
...
if (!function_exists('ajaxcontact_send_mail')) {
    ...
    function ajaxcontact_send_mail() {
        ...
        do_action('jobhunt_before_candidate_message_send', $_POST);
...
if (!function_exists('cs_add_applied_job_withoutlogin_to_usermeta')) {
    ...
    function cs_add_applied_job_withoutlogin_to_usermeta() {
        ...
        if ((isset($_FILES['media_upload']) && $_FILES['media_upload'] == '') || (!isset($_FILES['media_upload']) )) {
            $response['status'] = 0;
            $response['msg'] = sprintf(esc_html__("Please add your CV", 'jobhunt'));
        ...
        if ($cs_user_apply_job == 'on' && $package_apply_check) {
            ...
            if (username_exists($fullname) == null && email_exists($email) == false) {
                ...
                $username = $parts[0];
                $rand = rand(11, 9999);
                $user_id = wp_create_user($username . '-' . $rand, $phone, $rand.'_'.$email);
				
				update_user_meta($user_id, 'cs_phone_number', $phone);
...
if (!function_exists('cs_add_applied_job_to_usermeta')) {
    ...
    function cs_add_applied_job_to_usermeta() {
        ...
        $is_pckage = false;
	if ((isset($_FILES['media_upload']) && $_FILES['media_upload'] == '') || (!isset($_FILES['media_upload']) )) {
            if( !isset( $_POST['cs_candidate_cv'] ) || $_POST['cs_candidate_cv'] == ''){
                $response['status'] = 0;
                $response['msg'] = sprintf(esc_html__("Please add your CV", 'jobhunt'));
                echo json_encode($response);
                die;
            ...
            if ((isset($_POST['post_id']) && $_POST['post_id'] <> '')) {
                ...
                if ($dealine_response['status'] == 1) {
                    ...
                    if (isset($cs_wishlist) && in_array($_POST['post_id'], $cs_wishlist)) {
                        ...
                        $cs_wishlist = array_diff($post_id, $cs_wishlist);
                        cs_update_user_jobapply_meta($cs_wishlist);
                        cs_update_job_application_status( $post_id, $user);
                        $response['status'] = 1;
                        if ($view == 'modern-v1') {
                            $response['msg'] = esc_html__('Applied', 'jobhunt');
                        } else {
                    ...
                    if (isset($_FILES['media_upload']['name']) && !empty($_FILES['media_upload']['name']) && $_FILES['media_upload']['name'] != 'undefined') {
                        ...
                        if (isset($cs_media_image['error']) && $cs_media_image['error'] == 1) {
                            ...
                            update_user_meta($user, 'cs_candidate_cv' , $cs_media_image);
                        }
                    ...
                    cs_create_user_meta_list($_POST['post_id'], 'cs-user-jobs-applied-list', $user);
                    cs_update_job_application_status( $_POST['post_id'], $user);
                    do_action('jobhunt_applied_multi_jobs', $_POST['post_id'], $user);
                    $cs_email_template_atts = array(
                        'candidate_id' => $user,
                        'job_id' => $_POST['post_id'],
                    ...
                    $cs_connects_args = array(
                        'candidate_id' => $user,
                        'job_id' => $_POST['post_id'],
                        'trans_id' => isset($cs_package_check['cs_trans_id']) ? $cs_package_check['cs_trans_id'] : 0,
                    );
...
if (!function_exists('cs_get_custom_locationswith_google_auto')) {
    ...
    function cs_get_custom_locationswith_google_auto($dropdown_start_html = '', $dropdown_end_html = '', $cs_text_ret = false, $cs_top_search = false) {
        ...
        $output = '';
        $autocomplete_class = ( $cs_job_map_type == 'google')? 'cs_search_location_field' : '';
        ...
        $cs_opt_array = array(
            ...
            'echo' => false,
            'after' => '',
            'classes' => 'form-control '.$autocomplete_class,
    ...
    function cs_get_google_autocomplete_fields($type, $job_id = 0) {
        ...
        if( $cs_job_map_type == 'llmaps'){
            $output .= '<script type="text/javascript">
                var AutoCompleteConfigs = {
                    inputInitID: \'autocomplete_country\',
                }
                OSMAPAutoComplete(AutoCompleteConfigs);
                
                var AutoCompleteConfigs = {
                    inputInitID: \'autocomplete_city\',
                }
                OSMAPAutoComplete(AutoCompleteConfigs);
                
                var AutoCompleteConfigs = {
                    inputInitID: \'autocomplete_address\',
                }
                OSMAPAutoComplete(AutoCompleteConfigs);
                </script>';
        }else{
...
if (!function_exists('cs_find_other_field_user_meta_list')) {
    ...
    function cs_find_other_field_user_meta_list($post_id, $post_column, $list_name, $need_find, $user_id) {
        ...
        if (isset($finded[0])) {
            ...
            $need_find_value = isset($existing_list_data[$index][$need_find]) ? $existing_list_data[$index][$need_find] : '';
        }
...
if (!function_exists('get_user_address_string_for_list')) {
    ...
    function get_user_address_string_for_list($post_id, $type = 'post') {
        ...
        if ($type == 'post') {
            ...
            $cs_post_loc_country = isset($selected_spec->name) ? $selected_spec->name : $cs_post_loc_country;
            ...
            $cs_post_loc_region = isset($selected_spec->name) ? $selected_spec->name : $cs_post_loc_region;
            $cs_post_loc_city = get_post_meta($post_id, 'cs_post_loc_city', true);
            $selected_spec = get_term_by('slug', $cs_post_loc_city, 'cs_locations');
            $cs_post_loc_city = isset($selected_spec->name) ? $selected_spec->name : $cs_post_loc_city;
        } else {
...
if (!function_exists('cs_job_map')) {
    ...
    function cs_job_map($atts, $content = "") {
        ...
        $html .= "<script type='text/javascript'>
                    ...
                    var cs_map_type = '".$cs_job_map_type."';
                    ...
                    if( cs_map_type == 'llmaps'){
                        ...
                        llmapsObject.llmapsInitiate('map_canvas" . $map_dynmaic_no."');
...
if (!function_exists('jobhunt_applyjob_without_login_callback')) {
    ...
    function jobhunt_applyjob_without_login_callback($view) {
        ...
        if ($user_ID != '') {
            ...
            $user_fullname = isset( $user_info->display_name )? $user_info->display_name : '';
                                                    ...
                                                    <div class="dragareamain" style="padding-bottom:0px;">
                                                        ...
                                                        <div class="fileUpload uplaod-btn btn csborder-color cs-color">
                                                            ...
                                                            <label class="browse-icon">
                                                                ...
                                                                if(isset($cs_candidate_cv) && !empty($cs_candidate_cv)){
                                                                    $cs_opt_array['classes'] = 'upload cs-uploadimg cs-color csborder-color';
                                                                }else{
                                                                    $cs_opt_array['classes'] = 'upload cs-uploadimg cs-color csborder-color cs-required';
                                                                }
                                                                $cs_form_fields2->cs_form_text_render($cs_opt_array);
                                                                ?>
                                                            </label>
...
if (!function_exists('jobhunt_signup_direct_login_callback')) {
    ...
    function jobhunt_signup_direct_login_callback($json = array(), $login_data = array()) {
        ...
        if (is_wp_error($status)) {
            ...
            $uid = $status->ID;
            $cs_user_name = isset($_POST['user_login']) ? $_POST['user_login'] : '';
                        ...
                        <div class="cs-text">
                            <div class="content-holder">
                                ...
                                <div class="post-options"> 
                                    <?php $jobs_locations_enabled = apply_filters('jobhunt_jobs_locations_enabled', 'yes');
    ...
    function jobhunt_save_candidate_full_profile($candidate_id = '', $data = array()) {
        ...
        if (isset($candidate_query->results)) {
            foreach ($candidate_query->results as $candidate) {
                ...
                $cs_job_title = get_user_meta($candidate_id, 'cs_job_title', true);
                $cs_minimum_salary = get_user_meta($candidate_id, 'cs_minimum_salary', true);
                $cs_post_comp_address = get_user_meta($candidate_id, 'cs_post_comp_address', true);
                
                if (isset($cs_job_title) && $cs_job_title != '') {
                    $candidate_data[] = $cs_job_title;
                }
                if (isset($cs_minimum_salary) && $cs_minimum_salary != '') {
                    $candidate_data[] = $cs_minimum_salary;
                }
                if (isset($cs_post_comp_address) && !empty($cs_post_comp_address)) {
                    $candidate_data[] = $cs_post_comp_address;
                }
                $cs_specialisms = get_user_meta($candidate_id, 'cs_specialisms', true);
                if (isset($cs_specialisms) && !empty($cs_specialisms)) {
                    foreach ($cs_specialisms as $specialism) {
                ...
                $result = apply_filters('jobhunt_professions_candidate_search_data', isset($cs_array_data['cs_candidate_profession']) ? $cs_array_data['cs_candidate_profession'] : '');
                if (!empty($result)) {
                    $candidate_data[] = $result;
                }
                
                $cs_candidate_profession = get_user_meta($candidate_id, 'cs_candidate_profession', true);
                if (isset($cs_candidate_profession) && !empty($cs_candidate_profession)) {
                    foreach ($cs_candidate_profession as $professions) {
                        $professions_arr = get_term_by('slug', $professions, 'professions');
                        if (isset($professions_arr) && is_object($professions_arr)) {
                            $candidate_data[] = $professions_arr->name;
                        }
                    }
                }else if (isset($cs_array_data['cs_candidate_profession']) && !empty($cs_array_data['cs_candidate_profession'])) {
                    foreach ($cs_array_data['cs_candidate_profession'] as $professions) {
                        $professions_arr = get_term_by('slug', $professions, 'professions');
                        if (isset($professions_arr) && is_object($professions_arr)) {
                            $candidate_data[] = $professions_arr->name;
                        }
                ...
                $cs_award_name_array = get_user_meta($candidate_id, 'cs_award_name_array', true);
                if (isset($cs_award_name_array) && !empty($cs_award_name_array)) {
                    foreach ($cs_award_name_array as $award_name) {
                        $candidate_data[] = $award_name;
                    }
                }
                $cs_image_title_array = get_user_meta($candidate_id, 'cs_image_title_array', true);
                if (isset($cs_image_title_array) && !empty($cs_image_title_array)) {
                    foreach ($cs_image_title_array as $portfolio) {
                        $candidate_data[] = $portfolio;
                    }
                }
                $cs_edu_title_array = get_user_meta($candidate_id, 'cs_edu_title_array', true);
                if (isset($cs_edu_title_array) && !empty($cs_edu_title_array)) {
                    foreach ($cs_edu_title_array as $education) {
                        $candidate_data[] = $education;
                    }
                }
                $cs_exp_title_array = get_user_meta($candidate_id, 'cs_exp_title_array', true);
                if (isset($cs_exp_title_array) && !empty($cs_exp_title_array)) {
                    foreach ($cs_exp_title_array as $experience) {
                        $candidate_data[] = $experience;
                    }
                }
                $cs_cover_letter = get_user_meta($candidate_id, 'cs_cover_letter', true);
                if (isset($cs_cover_letter) && $cs_cover_letter != '') {
                    $candidate_data[] = str_replace(array('<p>', '</p>'), array('', ''), $cs_cover_letter);
                }
                update_user_meta($candidate_id, 'cs_candidate_search_flag', 1);
...
if (!function_exists('cs_update_job_application_status')) {
    function cs_update_job_application_status( $job_id = 0, $user_id = 0 ){
        update_user_meta($user_id, 'cs_job_application_status_'.$job_id, 'in-progress');
    }
}

if (!function_exists('cs_update_job_application_statuses')) {
    function cs_update_job_application_statuses( $status = '' ){
        
        $statuses['in-progress']          = esc_html('In Progress');
        $statuses['shortlisted']          = esc_html('Shortlisted');
        $statuses['contacted']            = esc_html('Contacted');
        $statuses['rejected']             = esc_html('Rejected');
        $statuses['interview-scheduled']  = esc_html('Interview Scheduled');
        $statuses['selected']             = esc_html('Selected');
        $statuses['hired']                = esc_html('Hired');
        
        $statuses = apply_filters('jobhunt_job_application_statuses', $statuses); 
...
function cs_update_application_status_callback(){
    ...
    if( $user_id > 0 && $job_id > 0 && $status != '' && is_user_logged_in()){
        update_user_meta($user_id, 'cs_job_application_status_'.$job_id, $status);
        
        $cs_job_employer = get_post_meta($job_id, "cs_job_username", true);
        $cs_job_employer = cs_get_user_id_by_login($cs_job_employer);
        do_action('job_application_status_notification', array( 'job_id' => $job_id, 'candidate_id' => $user_id, 'user_id' => $cs_job_employer, 'status' => $status ));
        
        $response['type'] = 'success';
        $response['msg']  = esc_html__('Application status updated successfully.');
        wp_send_json($response);
    }else{
        $response['type'] = 'error';
        $response['msg']  = esc_html__('Something went wrong.');
        wp_send_json($response);
    }
}

add_action('wp_ajax_cs_update_application_status', 'cs_update_application_status_callback');
add_action('wp_ajax_nopriv_cs_update_application_status', 'cs_update_application_status_callback');

if (!function_exists('jobhunt_editor_buttons_callback')) {
    if(!is_admin()){
        add_filter( 'teeny_mce_buttons', 'jobhunt_editor_buttons_callback', 30, 2 );
        function jobhunt_editor_buttons_callback( $buttons, $editor_id ) {
            return array( 'undo redo bold italic underline strikethrough fontselect fontsizeselect formatselect alignleft aligncenter alignright alignjustify outdent indent numlist bullist forecolor backcolor removeformat charmap link ltr rtl' );
        }
    }
}
if (!function_exists('jobhunt_editor_plugins_callback')) {
    if(!is_admin()){
        add_filter( 'teeny_mce_plugins', 'jobhunt_editor_plugins_callback', 30, 2 );
        function jobhunt_editor_plugins_callback( $plugins, $editor_id ) {
            return array( 'charmap colorpicker compat3x directionality fullscreen hr image link lists media paste tabfocus textcolor wordpress wpautoresize  wpdialogs wpeditimage wpemoji wpgallery wplink wptextpattern wpview' );
        }
    }
}

if (!function_exists('update_user_rich_editor_callback')) {
    add_action( 'init', 'update_user_rich_editor_callback', 30 );
    function update_user_rich_editor_callback(){
        $users = get_users( 
            array( 
                'role__in'   => array( 'cs_employer', 'cs_candidate'),
                'fields'     => 'ids',
                'meta_query' => array(
                    array(
                        'key'      => 'rich_editing',
        ...
        if(isset($users) && !empty($users)){
            foreach($users as $user){
                update_user_meta($user, 'rich_editing', 'true');
            }
        }
    }
}