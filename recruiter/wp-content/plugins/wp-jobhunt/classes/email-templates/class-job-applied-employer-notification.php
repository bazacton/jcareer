...
if ( ! class_exists('job_appied_employer_notification_template') ) {
    ...
    class job_appied_employer_notification_template {
        ...
        public function job_apply_employer_callback($arg) {
            ...
            $attachment = get_user_meta($candidate_id, 'cs_candidate_cv', true);
            if($attachment == ''){
                $attachment = get_user_meta($candidate_id, 'cs_candidate_cv_'.$jobhunt_post_id, true);
            }
            if(!empty($attachment)){
                $file_name = end(explode('uploads', $attachment));
                $attachment = array(WP_CONTENT_DIR . '/uploads'.$file_name);
            }
