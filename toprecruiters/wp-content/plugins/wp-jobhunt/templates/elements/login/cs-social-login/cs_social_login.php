...
// set parse request
    function cs_parse_request($wp) {
        ...
        $twitter_callback = '';
        if (isset($url['path']) && $url['path'] == '/social-login=twitter-callback') {
            $twitter_callback = 'yes';
        }
        if (array_key_exists('social-login', $wp->query_vars) || $twitter_callback == 'yes') {
            if (!session_id()) {
                session_start();
            }
            ...
            if (isset($wp->query_vars['social-login']) && $wp->query_vars['social-login'] == 'twitter') {
                ...
                cs_twitter_connect();
            } else if (isset($wp->query_vars['social-login']) && $wp->query_vars['social-login'] == 'twitter-callback' || $twitter_callback == 'yes') {
                cs_twitter_callback();
