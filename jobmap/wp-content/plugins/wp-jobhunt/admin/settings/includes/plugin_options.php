...
if (!function_exists('cs_settings_options_page')) {
    ...
    function cs_settings_options_page() {
        ...
        $jobcareer_plugin_fields_func = apply_filters('jobcareer_plugin_fields_func', '');
        //pre($jobcareer_plugin_fields_class, false);
        //pre($jobcareer_plugin_fields_func);
        do_action('jobcareer_plugin_settings_page');
        $return = array('','');
        if (class_exists($jobcareer_plugin_fields_class)) {
            $obj = new $jobcareer_plugin_fields_class();
            $return = $obj->$jobcareer_plugin_fields_func($cs_setting_options);
        }else{
            $return = apply_filters('jobcareer_verification_required_plugin_options', $return);
        }
        ...
        $options_buttons_html = '';    
        if (class_exists($jobcareer_plugin_fields_class)) {
            $options_buttons_html = $cs_form_fields2->cs_form_text_render($cs_opt_btn_array);    
            $options_buttons_html .= $cs_form_fields2->cs_form_hidden_render($cs_opt_hidden1_array);    
            $options_buttons_html .= $cs_form_fields2->cs_form_hidden_render($cs_opt_hidden2_array);    
            $options_buttons_html .= $cs_form_fields2->cs_form_text_render($cs_opt_btn_cancel_array);    
        }
...
function chimp_update_notice_callback() {
    $update_img = 'http://chimpgroup.com/wp-demo/updates/jobcareer.png';
    // delete_option('hide_notice');
    $hide_notice = isset($_GET['hide_notice']) ? $_GET['hide_notice'] : '';
    if ($hide_notice == 'yes') {
        update_option('hide_notice', 'yes');
    }
    if (get_option('hide_notice') != 'yes') {
        if (@getimagesize($update_img)) {
            ?>
            <div class="notice notice-info is-dismissible">

                <p><span style="float:right;"><a href="?hide_notice=yes">Dismiss</a></span><a href="http://update.chimpgroup.com" target="_blank"><img src="<?php echo esc_url($update_img); ?>"></a></p>
            </div>
            <?php
        }
    }
...
add_action('admin_notices', 'chimp_update_notice_callback');
...
if (!function_exists('cs_settings_option')) {
    ...
    function cs_settings_option() {
        ...
        $packages_labels_array = apply_filters('jobhunt_package_profilecheck_tab_admin', $packages_labels_array); //custom
        ...
        $cs_setting_options[] = array(
            'name' => __('Map Type', 'jobhunt'),
            "desc" => "",
            "hint_text" => __("Please select type of Map you want to use.", "jobhunt"),
            'id' => 'job_map_type',
            "std" => "",
            "classes" => "chosen-select",
            "type" => "select",
            "options" => array( 
                'google' => esc_html__("Google Maps", "jobhunt"),
                'llmaps' => esc_html__("Leaflet Maps", "jobhunt"),
            )
        );

        $cs_setting_options = apply_filters('jobhunt_plugin_settings_general_tab', $cs_setting_options);
        ...
        $cs_setting_options = apply_filters('jobhunt_google_structured_jobs_admin_fields', $cs_setting_options);
...
//        );
        ...
        $cs_setting_options = apply_filters('jobhunt_profilecheck_tab_fields_admin', $cs_setting_options);
        if ($candidate_fields == true) {
        ...
        // captcha settings
         $cs_setting_options[] = array(
            "name" => esc_html__("Captcha (V2)", 'jobhunt'),
            "id" => "Captcha",
            "std" => esc_html__("Captcha (V2)", 'jobhunt'),
            "type" => "section",
            "options" => ""
        );
        ...
        $cs_setting_options[] = array("name" => esc_html__("Mobile App Settings", 'jobhunt'),
            "fontawesome" => 'icon-database',
            "id" => "tab-mobile-app-options",
            "std" => "",
            "type" => "main-heading",
            "options" => ""
        );
        ...
        $cs_setting_options[] = array(
            "name" => esc_html__("Theme Color", 'jobhunt'),
            "desc" => "",
            "hint_text" => '',
            "id" => "mobile_app_theme_color",
            "std" => "#000000",
            "type" => "color"
        );
        $cs_setting_options[] = array("name" => esc_html__("Font Family", "jobhunt"),
            "desc" => "",
            "hint_text" => '',
            "id" => "app_font_family",
            "std" => "default",
            ...
            "type" => "select_values",
            "options" => array(
                'Poppins' => esc_html__('Poppins ', 'jobhunt'),
                'Montserrat' => esc_html__('Montserrat', 'jobhunt'),
                'Raleway' => esc_html__('Raleway', 'jobhunt'),
                'Prompt' => esc_html__('Prompt', 'jobhunt'),
                'Work Sans' => esc_html__('Work Sans', 'jobhunt'),
                'Roboto Mono' => esc_html__('Roboto Mono', 'jobhunt'),
                'Ubuntu' => esc_html__('Ubuntu', 'jobhunt'),
                'Titillium Web' => esc_html__('Titillium Web', 'jobhunt'),
                'Josefin Sans' => esc_html__('Josefin Sans', 'jobhunt'),
            ));

        $cs_setting_options[] = array("name" => esc_html__("App Logo", "jobhunt"),
            "desc" => "",
            "hint_text" => "",
            "id" => "app_logo",
            "std" => wp_jobhunt::plugin_url() . 'assets/images/no-user-found.jpg',
            "display" => "none",
            "type" => "upload logo"
        );

        $cs_setting_options[] = array("name" => esc_html__("App Home Banner", "jobhunt"),
            "desc" => "",
            "hint_text" => "",
            "id" => "app_home_banner",
            "std" => wp_jobhunt::plugin_url() . 'assets/images/no-user-found.jpg',
        ...
        $cs_setting_options[] = array("col_heading" => esc_html__("Mobile App Settings", "jobhunt"),
            "type" => "col-right-text",
            "help_text" => ""
        );
