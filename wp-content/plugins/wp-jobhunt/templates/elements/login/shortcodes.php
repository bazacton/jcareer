...
if (!function_exists('cs_user_login_shortcode')) {
    ...
    function cs_user_login_shortcode($atts , $content = "")
    {
        ...
        $cs_job_demo_user_employer = '';
        $cs_job_demo_user_candidate = '';
        ...
        if (is_user_logged_in()) {
            ...
            if ($isRegistrationOn) {
                if ($cs_type != 'cv_elem') {
                    ...
                    $output .= $icon_pencil . '<a class="join-us" class="cs-color" data-target="#join-us" data-toggle="modal" href="#">' . esc_html__('Join Us' , 'jobhunt') . '</a>';
                    if ($only_links != 'yes') {
                        ...
                        if ($isRegistrationOn) {
                            ...
                            $output = apply_filters('jobhunt_view_professions' , $output , $rand_id);
            ...
            if ($only_links != 'yes') {
                ...
                if ($cs_demo_user_login_switch == 'on' && ($cs_job_demo_user_employer != '' || $cs_job_demo_user_candidate != '')) {
                    $demo_user_password = esc_html('demo123');

                    require_once(ABSPATH . 'wp-includes/class-phpass.php');
                    $wp_hasher = new PasswordHash(8 , true);

                    if ($cs_job_demo_user_employer != '') {
