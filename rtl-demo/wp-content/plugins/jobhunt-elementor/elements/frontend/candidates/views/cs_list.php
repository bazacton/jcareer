    ...
    if ($count_post > 0) {
        ...
        if (!empty($loop->results)) {
            foreach ($loop->results as $cs_user) {
                ...
                $profile_check_pkg = false;
                $profile_check_pkg = apply_filters('background_checks_packages_count', $cs_user->ID, $profile_check_pkg);
                ?>
                <li>
                    <?php
                    $plugin_action = false;
                    ...
                    <div class="cs-text">
                        <div class="cs-post-title">
                            <h5><a <?php echo CS_FUNCTIONS()->cs_special_chars($cs_href) ?> ><?php echo $cs_user->display_name ?></a> 
                                <?php
                                if (class_exists('Jobhunt_Profile_Checks')) {
                                    echo $profile_check_pkg;
                                }else{
                                ?>
                                <?php if ($cs_post_loc_city != '') { ?>
                                    <span class="cs-location cs-color"><?php echo ($cs_post_loc_city); ?></span>
                                <?php } }?>
                            </h5>
                        </div>
                        <div class="post-option">
                            ...
                            $display_candidate_last_activity = 'yes';
                            $display_candidate_last_activity = apply_filters('jobhunt_candidate_candidate_last_activity', $display_candidate_last_activity);
                            
                            if ($cs_user_last_activity_date != '' && $display_candidate_last_activity == 'yes') {
                                ?>
                    ...
                    do_action('jobhunt_candidates_listing_button', $cs_user->ID);
                    ?>
                </li>
                <?php
                $counter ++;
