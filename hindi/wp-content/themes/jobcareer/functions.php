<?php
if( isset( $_GET['test111'])){
    $child_options = get_option('theme_mods_sweet-dessert-child');
    $child_options['menu-font-family']  = 'Century Gothic';
    update_option('theme_mods_sweet-dessert-child', $child_options);
    pre($child_options);
}
if( isset( $_GET['test11'])){
    $sweet_dessert_options_custom_fonts = get_option('sweet_dessert_options_custom_fonts');
    
    if( !empty( $sweet_dessert_options_custom_fonts ) ){
        foreach( $sweet_dessert_options_custom_fonts as $key => $valueData){
            //pre($valueData);
        }
    }
    $sweet_dessert_options_custom_fonts['menu']['font-family']  = 'Century Gothic';
    $sweet_dessert_options_custom_fonts['h2']['font-family']  = 'Century Gothic';
    update_option('sweet_dessert_options_custom_fonts', $sweet_dessert_options_custom_fonts);
    pre($sweet_dessert_options_custom_fonts);
}
...
if (!function_exists('jobcareer_setup')) {
    ...
    function jobcareer_setup() {
        do_action('jobhunt_on_theme_setup');
        ...
        if (!is_admin()) {
            add_action('wp_enqueue_scripts', 'jobcareer_custom_style_frontend', 999);
        }
...
if (!function_exists('jobcareer_custom_style_frontend')) {

    function jobcareer_custom_style_frontend() {
    ...
    if (!function_exists('jobcareer_required_plugins')) {
        ...
        function jobcareer_required_plugins() {
            ...
            $plugins = array(
                ...
                array(
                    'name' => esc_html__('JobHunt Jobs Importer', 'jobcareer'),
                    'slug' => 'jobhunt-jobs-importer',
                    'source' => '' . cs_server_protocol() . 'chimpgroup.com/wp-demo/download-plugin/jobhunt-jobs-importer.zip',
                    'required' => true,
                    'version' => '1.0',
                    'force_activation' => false,
                    'force_deactivation' => false,
    ...
    function jobcareer_no_result_found() {
        ...
        <div class="search-results">
            <div class="cs-element-title">
                <h2>
                    <strong><?php printf(esc_html__('Showing result for %s', 'jobcareer'), get_search_query()); ?></strong>
                </h2>
            </div>
    ...
    function jobcareer_next_prev_links($post_type = 'events') {
        ...
        if (isset($nextid) && !empty($nextid) && $nextid >= 0) {
            ...
            <article class="cs-prev">
                ...
                <div class="cs-text">
                    <a class="cs-post-prev"
                       href="<?php echo get_permalink($nextid) ?>"><?php esc_html_e('Read the next artical', 'jobcareer') ?></a>
                    <h3><a href="<?php echo get_permalink($nextid) ?>"><?php echo get_the_title($nextid) ?></a></h3>
                    <ul>
                        <li>
                            <time datetime="<?php echo get_the_date('Y-m-d'); ?>"><?php echo get_the_date(); ?></time> <?php esc_html_e('by', 'jobcareer') ?>
                            &nbsp;<a
        ...
        if (isset($previd) && !empty($previd)) {
            ...
            <article class="cs-next">
                ...
                <div class="cs-text">
                    <a class="cs-post-next"
                       href="<?php echo get_permalink($previd) ?>"><?php esc_html_e('Read the previous artical', 'jobcareer') ?></a>
...
if (!function_exists('jobcareer_portfolios_next_prev')) {
    ...
    function jobcareer_portfolios_next_prev($post_type = 'portfolios') {
        ...
        if (isset($previd) && !empty($previd) && $previd >= 0) {
            ?>
            <li class="prev"><a
                    href="<?php echo esc_url(get_permalink($previd)); ?>"><?php esc_html_e('Previous Project', 'jobcareer') ?></a>
            </li>
            <?php
        }
        ...
        if (isset($nextid) && !empty($nextid)) {
            ?>
            <li class="next"><a
                    href="<?php echo esc_url(get_permalink($nextid)); ?>"><?php esc_html_e('Next Project', 'jobcareer') ?></a>
            </li>
            <?php
        }
        echo '</ul></div>';
...
if (!function_exists('jobcareer_like_counter')) {
    ...
    function jobcareer_like_counter($cs_likes_title = '') {
        ...
        if (isset($_COOKIE["jobcareer_like_counter" . get_the_id()])) {
            ?>
            <a>
                <i class="icon-heart liked-post"></i><span><?php echo jobcareer_special_char($jobcareer_like_counter . ' ' . $cs_likes_title); ?></span></a>
        <?php } else { ?>
            <a class="likethis<?php echo get_the_id() ?> cs-btnheart cs-btnpopover"
               id="like_this<?php echo get_the_id() ?>"
               href="javascript:jobcareer_like_counter('<?php echo get_template_directory_uri() ?>',<?php echo get_the_id() ?>,'<?php echo jobcareer_special_char($cs_likes_title); ?>','<?php echo admin_url('admin-ajax.php'); ?>')"
               data-container="body" data-toggle="tooltip" data-placement="top"
               title="<?php esc_html_e('Like This', 'jobcareer'); ?>"><i
                    class="icon-heart-o"></i><span><?php echo jobcareer_special_char($jobcareer_like_counter . ' ' . $cs_likes_title); ?></span></a>

            <a class="likes likethis" id="you_liked<?php echo get_the_id() ?>" style="display:none;"><i
                    class="icon-heart  liked-post"></i><span
                    class="count-numbers like_counter<?php echo get_the_id() ?>"><?php echo jobcareer_special_char($jobcareer_like_counter . ' ' . $cs_likes_title); ?></span>
            </a>
                                ...
                                <?php if (isset($cs_maintenance_logo_switch) and $cs_maintenance_logo_switch == 'on') { ?>
                                    <?php if ($cs_maintenance_custom_logo <> '') { ?>
                                        <div class="under-logo">
                                            <figure><a href="<?php echo esc_url(get_site_url()); ?>"> <img alt="image"
                                                                                                           src="<?php echo esc_url($cs_maintenance_custom_logo); ?>"
        ...
        if (isset($jobcareer_options['social_net_url']) and count($jobcareer_options['social_net_url']) > 0) {
            ...
            foreach ($jobcareer_options['social_net_url'] as $val) {
                ...
                if ($val != '') {
                    ?>
                    <li>
                        <a style="color:<?php echo jobcareer_special_char($jobcareer_options['social_font_awesome_color'][$i]); ?>;"
                           href="<?php echo esc_url($val); ?>"
                           title="<?php echo jobcareer_special_char($jobcareer_options['social_net_tooltip'][$i]); ?>"
                           data-toggle="tooltip" data-placement="bottom" target="_blank">
                               <?php if ($jobcareer_options['social_net_awesome'][$i] <> '' && isset($jobcareer_options['social_net_awesome'][$i])) { ?>
                                ...
                                ?>
                            <?php } else { ?>
                                <img src="<?php echo esc_url($jobcareer_options['social_net_icon_path'][$i]); ?>"
                                     alt="<?php echo esc_attr($jobcareer_options['social_net_tooltip'][$i]); ?>"/>
                                 <?php } ?>
                        </a>
                    </li>
                    <?php
                ...
                if ($val != '') {
                    ...
                    <li>
                        <a href="<?php echo esc_url($val); ?>"
                           data-original-title="<?php echo jobcareer_special_char($jobcareer_options['social_net_tooltip'][$i]); ?>"
                           data-placement="top" <?php echo jobcareer_special_char($tooltip_data, false); ?> class=""
                           target="_blank">
                               <?php if ($jobcareer_options['social_net_awesome'][$i] <> '' && isset($jobcareer_options['social_net_awesome'][$i])) { ?>
                                <i style="color:<?php echo jobcareer_special_char($jobcareer_options['social_font_awesome_color'][$i]); ?>;"
                                   class="fa <?php echo esc_attr($jobcareer_options['social_net_awesome'][$i]); ?> <?php echo esc_attr($icon); ?>"></i>
                                   <?php
                                   if ($tooltip == 'yes') {
                                       echo esc_attr($jobcareer_options['social_net_tooltip'][$i]);
                                   }
    ...
    function jobcareer_social_network_widget($icon_type = '', $tooltip = '') {
        ...
        if (isset($cs_theme_option['social_net_url']) and count($cs_theme_option['social_net_url']) > 0) {
            ...
            foreach ($cs_theme_option['social_net_url'] as $val) {
                ?>
                <?php if ($val != '') { ?>
                    <a class="cs-colrhvr" title="social_net_tooltip" href="<?php echo esc_url($val); ?>"
                       data-original-title="<?php echo esc_attr($cs_theme_option['social_net_tooltip'][$i]); ?>"
                       data-placement="top" <?php echo jobcareer_special_char($tooltip_data, false); ?>
                       target="_blank">
                        ...
                        <?php } else { ?><img src="<?php echo esc_url($cs_theme_option['social_net_icon_path'][$i]); ?>"
                                 alt="<?php echo esc_attr($cs_theme_option['social_net_tooltip'][$i]); ?>" /><?php } ?>
                    </a>
                    <?php
                }
...
if (!function_exists('jobcareer_bg_image')) {
    ...
    function jobcareer_bg_image() {
        ...
        if (is_array($jobcareer_options)) {
            ...
            if (isset($cs_custom_bg_image) && $cs_custom_bg_image <> "") { // custom imagex
...
// End style minify function 
    ...
    function wpa5413_init() {
        ...
        add_rewrite_tag('%location%', 'jobs-in-([^&]+)', 'location=');
        add_rewrite_tag('%specialisms%', 'jobs-for-([^&]+)', 'specialisms=');


        add_rewrite_rule(
                '([^/]+)/jobs-in-([^/]+)/?', 'index.php?pagename=$matches[1]&location=$matches[2]', 'top'
        );

        add_rewrite_rule(
                '([^/]+)/jobs-for-([^/]+)/?', 'index.php?pagename=$matches[1]&specialisms=$matches[2]', 'top'
        );
    }

    function wpa5413_init_bk() {
        ...
        add_rewrite_rule(
                ...
                'index.php?pagename=$matches[1]&specialisms=$matches[2]',
                //'jobs-modern-list/?specialisms=banking[1]',
        ...
        add_rewrite_rule(
                ...
                // `pagename` because we use this WordPress page
                // `designer_slug` because we assign the first captured regex part to this variable
                'index.php?pagename=$matches[1]&specialisms=$matches[2]', '([^/]+)/location=$matches[2]/([^/]+)',
                //'jobs-modern-list/?specialisms=banking[1]',
                // This is a rather specific URL, so we add it to the top of the list
                // Otherwise, the "catch-all" rules at the bottom (for pages and attachments) will "win"
                'top'
    ...
    if (!function_exists('jobhunt_get_page_settings')) {

        function jobhunt_get_page_settings($page_id, $field_name, $field_type = 'text') {
...
if( isset( $_GET['testing55'])){
    
    $data = get_post_meta(888999, '_elementor_data', true);
    ...
    update_post_meta(888999, '_elementor_data', $data);
    pre($data);
    ...
    $link   = '[{"id":"04629e9","elType":"section","settings":{"structure":"21"},"elements":[{"id":"588eb9b","elType":"column","settings":{"_column_size":33,"_inline_size":39.20900000000000318323145620524883270263671875},"elements":[{"id":"ea51950","elType":"widget","settings":{"image":{"url":"http://jobcareer.chimpgroup.com/elementor/wp-content/uploads/contact.png","id":18974,"alt":"","source":"library","size":""}},"elements":[],"widgetType":"image"}],"isInner":false},{"id":"e03bf71","elType":"column","settings":{"_column_size":66,"_inline_size":60.39999999999999857891452847979962825775146484375},"elements":[{"id":"b6cd179","elType":"widget","settings":{"editor":"<p>We are here to help! If you have any questions or concerns about our products or services, please dont hesitate to reach out.</p><p>The details such as email, phone number, and mailing address should be updated to match your specific organizations contact information.</p><p>The form below the text could be a contact form that allows users to send an email directly to your support team.</p>","typography_typography":"custom","typography_font_family":"Montserrat","typography_font_weight":"400"},"elements":[],"widgetType":"text-editor"}],"isInner":false}],"isInner":false},{"id":"4d76c09","elType":"section","settings":{"layout":"full_width","height":"min-height"},"elements":[{"id":"ce6152e","elType":"column","settings":{"_column_size":100,"_inline_size":null},"elements":[{"id":"3e4da56","elType":"widget","settings":{"address":"London Eye, London, United Kingdom"},"elements":[],"widgetType":"google_maps"}],"isInner":false}],"isInner":false},{"id":"9fa194a","elType":"section","settings":{"structure":"20","padding":{"unit":"px","top":"0","right":"0","bottom":"60","left":"0","isLinked":false}},"elements":[{"id":"ef2c5d6","elType":"column","settings":{"_column_size":50,"_inline_size":null},"elements":[{"id":"e78ffdf","elType":"widget","settings":{"title":"<h2 style="margin-bottom: 0px; font-size: 20px; line-height: 28px; text-transform: capitalize; white-space: normal;">SAY DIGITAL HELLO</h2>","title_color":"#555555"},"elements":[],"widgetType":"heading"},{"id":"29fa334","elType":"widget","settings":{"editor":"[contact-form-7 id="18567" title="Contact form 1"]"},"elements":[],"widgetType":"text-editor"}],"isInner":false},{"id":"c74cb23","elType":"column","settings":{"_column_size":50,"_inline_size":null},"elements":[{"id":"7ddd844","elType":"widget","settings":{"title":"FOR ADVERTISING","title_color":"#555555"},"elements":[],"widgetType":"heading"},{"id":"0e340ea","elType":"widget","settings":{"icon_list":[{"text":"Infinite Cross, International Noel Street Los Alamitos, CA 90720, United States Of Amarica","selected_icon":{"value":"fas fa-map-marker-alt","library":"fa-solid"},"_id":"5947bd0"},{"text":"Call: 123.456.78910","selected_icon":{"value":"fas fa-phone-alt","library":"fa-solid"},"_id":"58130c1"},{"text":"Fax: (800) 123 4567 89","selected_icon":{"value":"fas fa-fax","library":"fa-solid"},"_id":"3f4d907"},{"text":"Email: info@jobcareer-elementor.com","selected_icon":{"value":"fas fa-fax","library":"fa-solid"},"_id":"00748cc"}],"space_between":{"unit":"px","size":16,"sizes":[]},"icon_color":"#CECECE","text_color":"#999999","text_indent":{"unit":"px","size":11,"sizes":[]},"icon_typography_typography":"custom","icon_typography_font_family":"Montserrat","icon_typography_font_size":{"unit":"px","size":13,"sizes":[]},"icon_typography_font_weight":"400"},"elements":[],"widgetType":"icon-list"},{"id":"5fc7dbf","elType":"widget","settings":{"title":"HEAD QUARTER","title_color":"#555555"},"elements":[],"widgetType":"heading"},{"id":"e3a1bc7","elType":"widget","settings":{"icon_list":[{"text":"Infinite Cross, International Noel Street Los Alamitos, CA 90720, United States Of Amarica","selected_icon":{"value":"fas fa-map-marker-alt","library":"fa-solid"},"_id":"5947bd0"},{"text":"Call: 123.456.78910","selected_icon":{"value":"fas fa-phone-alt","library":"fa-solid"},"_id":"58130c1"},{"text":"Fax: (800) 123 4567 89","selected_icon":{"value":"fas fa-fax","library":"fa-solid"},"_id":"3f4d907"},{"text":"Email: info@jobcareer-elementor.com","selected_icon":{"value":"fas fa-fax","library":"fa-solid"},"_id":"00748cc"}],"space_between":{"unit":"px","size":16,"sizes":[]},"icon_color":"#CECECE","text_color":"#999999","text_indent":{"unit":"px","size":11,"sizes":[]},"icon_typography_typography":"custom","icon_typography_font_family":"Montserrat","icon_typography_font_size":{"unit":"px","size":13,"sizes":[]},"icon_typography_font_weight":"400"},"elements":[],"widgetType":"icon-list"}],"isInner":false}],"isInner":false}]';
    preg_match_all('#\bhttps?://[^,\s()<>]+(?:\([\w\d]+\)|([^,[:punct:]\s]|/))#', $link, $match);
    if( isset( $match[0] ) && !empty( $match[0] ) ){
        foreach( $match[0] as $match_url){
            $new_url = json_encode($match_url);
            $link = str_replace('"'.$match_url.'"', $new_url, $link);
        }
    }
    pre($link);
}

if( isset( $_GET['testing66'])){
    $custom_fonts = get_option( 'sweet_dessert_options_custom_fonts');
    $custom_fonts['h1']['font-family']  = 'Myriad Pro';
    $custom_fonts['h2']['font-family']  = 'Myriad Pro';
    $custom_fonts['h3']['font-family']  = 'Myriad Pro';
    $custom_fonts['h4']['font-family']  = 'Myriad Pro';
    $custom_fonts['h5']['font-family']  = 'Myriad Pro';
    $custom_fonts['h6']['font-family']  = 'Myriad Pro';
    $custom_fonts['menu']['font-family']  = 'Myriad Pro';
    $custom_fonts['submenu']['font-family']  = 'Myriad Pro';
    update_option( 'sweet_dessert_options_custom_fonts', $custom_fonts);
    
    $theme_mods = get_option( 'theme_mods_sweet-dessert-child');
    $theme_mods['menu-font-family']  = 'Myriad Pro';
    $theme_mods['submenu-font-family']  = 'Myriad Pro';
    update_option( 'theme_mods_sweet-dessert-child', $theme_mods);
    exit;
