...
//job status approved/not approved email hook
        ...
        public function reg_shortcodes_btn() {
            ...
            $shortcode_array = apply_filters('profilecheck_jobcareer_shortcode_names', $shortcode_array); //custom
                            ...
                            if (!$active_addon) {
                                ...
                                $cs_opt_array = array(
                                    ...
                                    'field_params' => array(
                                        ...
                                        'classes' =>  'chosen-select form-select-city dir-map-search single-select SlectBox',
                                        'extra_atr' => $extra_attr,
                ...
                (function ($) {
                    $(function () {
                        ...
                        if( cs_map_type == 'llmaps'){
                            var mapConfigs = {
                                center: ['<?php echo $cs_post_loc_latitude; ?>', '<?php echo $cs_post_loc_longitude; ?>'],
                                zoom: 5,
                                map_type: 'default',
                            }
                            ...
                            var markersArray = [];
                            var markerData = {
                                latitude: '<?php echo $cs_post_loc_latitude; ?>',
                                longitude: '<?php echo $cs_post_loc_longitude; ?>',
                            }

                            markersArray.push(markerData);
                            llmapsObject.registerMarkers(markersArray);
                            var LLMAPSObject = llmapsObject.llmapsInitiate('cs-map-location-id');


                            var AutoCompleteConfigs = {
                                inputInitID: 'loc_address',
                                callbackFunc: 'cs_autocomplete_search',
                            }

                            OSMAPAutoComplete(AutoCompleteConfigs);

                            window.cs_autocomplete_search = function(lat,lon,address) {
                                var mapObject = LLMAPSObject.mapObject;
                                jQuery("#cs_post_loc_latitude").val(lat);
                                jQuery("#cs_post_loc_longitude").val(lon);
                                mapObject.eachLayer(function (layerObj) {
                                    if (typeof layerObj.setLatLng != "undefined"){
                                        layerObj.setLatLng([lat,lon]);
                                    }
                                });
                                mapObject.panTo([lat,lon]);
                            } 
                            
                            var AutoCompleteConfigs = {
                                inputInitID: 'loc_country',
                            }
                            OSMAPAutoComplete(AutoCompleteConfigs);

                            var AutoCompleteConfigs = {
                                inputInitID: 'loc_city',
                            }
                            OSMAPAutoComplete(AutoCompleteConfigs);

                            var AutoCompleteConfigs = {
                                inputInitID: 'complete_address',
                            }
                            OSMAPAutoComplete(AutoCompleteConfigs);
                        ...
                        if( cs_map_type == 'google'){
                            ...
                            autocomplete = new google.maps.places.Autocomplete(jQuery('input[id="loc_city"]')[0]);
            ...
            <script>
                ...
                jQuery(document).ready(function () {
                    ...
                    if( cs_map_type == 'llmaps'){
                        ...
                        markersArray.push(markerData);
                        llmapsObject.registerMarkers(markersArray);
                        var LLMAPSObject = llmapsObject.llmapsInitiate('cs-map-location-fe-id');                  
                        ...
                        window.cs_ondragg = function (markerObject){
                            var LatLngObject = markerObject.getLatLng();
                            ...
                            jQuery.get('https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat='+latitude+'&lon='+longitude+'', function(data){
                                jQuery("#cs_post_loc_latitude").val(latitude);
                                jQuery("#cs_post_loc_longitude").val(longitude);
                                jQuery("#loc_address").val(data.display_name);
                            });
                        }
